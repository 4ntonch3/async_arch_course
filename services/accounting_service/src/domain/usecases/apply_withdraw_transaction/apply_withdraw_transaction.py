...
class ApplyWithdrawTransactionUsecase:
    def __init__(
        self,
    ...
    async def execute(self, task_public_id: str, worker_public_id: str) -> None:
        ...
        description = f"Fee for getting task. Task ID: {task_public_id}"
        transaction = await self._transactions_repository.apply_withdraw(
            worker_public_id, task.assign_fee, description
        )
