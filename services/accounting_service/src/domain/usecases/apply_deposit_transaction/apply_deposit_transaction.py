...
class ApplyDepositTransactionUsecase:
    def __init__(
        self,
    ...
    async def execute(self, task_public_id: str, worker_public_id: str, description: str) -> None:
        ...
        transaction = await self._transactions_repository.apply_deposit(
            worker_public_id, task.cost.assign_fee, description
        )

        await self._message_broker.produce_deposit_transactions_applied([transaction])
